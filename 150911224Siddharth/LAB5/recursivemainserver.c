...
int main()
{
	...
	serveraddr.sin_port=htons(3700);
	...
	{
		int flag = -1, i=0;
		rcvbytes=recvfrom(sockfd,hname,sizeof(hname),0,(struct sockaddr*)&serveraddr, &len);
		if(rcvbytes==-1)
		{
			close(sockfd);
		}
		...
		{
			strcpy(ip, d[flag].ip);
			sentbytes=sendto(sockfd,ip,sizeof(ip),0,(struct sockaddr*)&serveraddr, sizeof(serveraddr));
